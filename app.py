...
# TODO check errors and make it indempotent- login for user would be the best
def participate(event_id, user_name, user_surname):
    ...
    add_participation = Participate(event_id=event_id, user_name=user_name,user_surname=user_surname, code=code)
...
def registrate_to_event():
    if request.method == 'POST':
        ...
        print(request.get_json()['event_id'])
        return participate(event_id, user_name, user_surname)

@app.route("/api/participate", methods = ['DELETE'])
